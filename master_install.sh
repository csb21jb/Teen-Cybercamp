...
for tool in python3 curl sudo openssl git nmap wget bsdmainutils net-tools exiftool netcat-traditional openssh-server build-essential vim apt-transport-https zip ca-certificates software-properties-common ftp systemd gnupg lsb-release nano debconf-utils; do apt install -y "$tool" && sleep 1; done

sleep 2  # Wait for 2 seconds

echo -e "\n\n\n\n"  # Add two blank lines for spacing

####################################################### Download the CTF Install Script ##########################
sleep 2  # Wait for 2 seconds
